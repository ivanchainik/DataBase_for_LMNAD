...
class Appliances(models.Model):
    ...
    appliances_model = models.TextField('Модели', blank = True)
    appliances_paramerts = models.CharField('Параметры', max_length=300, blank = True)
    appliances_manufacturer = models.CharField('Производитель', max_length=300, blank = True)
...
class Expirement(models.Model):
    ...
    source = models.ForeignKey(Source, null=True, on_delete=models.PROTECT, verbose_name='Источник')
    ...
    appliance = models.ManyToManyField(Appliances, blank=True, verbose_name='Приборы')
    places = models.CharField('Научный центр', blank = True, max_length=250)
    measurable = models.ManyToManyField(Measurable, verbose_name='Начальные параметры')
    configurate_liquid = models.CharField('Конфигурация жидкости', null=True, max_length=200)
    effects = models.CharField('Тип эксперимента', max_length=300)
